...
# Aqui empieza la logica y funcionamiento principal del programa
    ...
    def ejecucion_principal(self):
            ...
            if self.window_4 is not None and self.estado3:
                ...
                self.frame_contador.grid(row=0, column=0, padx=(
                    5, 0), pady=(10, 10))
                ...
                self.frame_registros = tk.Frame(
                    self.frame_margen4, bd=2, height=349, width=309, relief="ridge")
                self.frame_registros.grid(row=1, column=0, padx=(4, 0), pady=2)

                # tamaño de la ventana secundaria
                self.window_width_4 = 330
                self.window_height_4 = 420
                self.window_4.geometry(
                    f"{self.window_width_4}x{self.window_height_4}+841+174")
                self.estado3 = True
    ...
    def abrir_ventana_secundaria(self):
        if self.window_4 is not None and self.estado3:
            ...
            #marco secundario instructivo
            self.frame_margen4 = tk.Frame(self.window_4, bd=2, relief=tk.SUNKEN)
            self.frame_margen4.pack(
                fill=tk.BOTH, expand=True, padx=5, pady=5)

            self.frame_contador = tk.Frame(
                self.frame_margen4, bd=2, highlightbackground="forest green", relief="sunken", highlightthickness=3)
            self.frame_contador.grid(row=0, column=0, padx=(
                5, 0), pady=(10, 10))

            # Label de conteo, busqueda y encontrados
            self.localizando = tk.Label(
                self.frame_contador, text=f"Localizando {self.c2} de {self.total_filas}")
            self.localizando.grid(row=0, column=0)
            self.encontrados_l = tk.Label(
                self.frame_contador, text=f"/Archivos Encontrados {self.c1}")
            self.encontrados_l.grid(row=0, column=1)
            self.frame_registros = tk.Frame(
                self.frame_margen4, bd=2, height=349, width=309, relief="ridge")
            self.frame_registros.grid(row=1, column=0, padx=(4, 0), pady=2)
...
def intructivo_uso():
    ...
    if window_2 is not None and estado1:
        ...
        window_width_2 = 307
        window_height_2 = 420
        window_2.geometry(f"{window_width_2}x{window_height_2}+220+174")

        # SCROLL para ventana de instrucion
...
def acerca_de():
    ...
    if window_3 is not None and estado2:
        ...
        window_width_3 = 307
        window_height_3 = 420
        window_3.geometry(f"{window_width_3}x{window_height_3}+220+174")
